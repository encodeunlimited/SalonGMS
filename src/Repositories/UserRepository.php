...
class UserRepository extends BaseRepository
{
    ...
    protected function getSearchableFields(): array { return ['name', 'user_name', 'email', 'role']; }
    ...
    public function getByIdentifierGlobal(string $identifier): ?array
    {
        $stmt = $this->db->prepare("SELECT * FROM {$this->table} WHERE (email = :identifier OR user_name = :identifier) LIMIT 1");
    ...
    public function create(array $data): array
    {
        $stmt = $this->db->prepare("
            INSERT INTO {$this->table} (tenant_id, role, name, user_name, email, password, commission_rate, profile_image, specialist_areas)
            VALUES (:tenant_id, :role, :name, :user_name, :email, :password, :commission_rate, :profile_image, :specialist_areas)
        ");
    ...
    {
        $sql = "UPDATE {$this->table} SET role = :role, name = :name, user_name = :user_name, email = :email, commission_rate = :commission_rate, specialist_areas = :specialist_areas";
